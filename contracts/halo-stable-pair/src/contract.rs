...
pub fn provide_liquidity(
    ...
    if total_share == Uint128::zero() {
        // mint LP_TOKEN_RESERVED_AMOUNT to the pair contract
        messages.push(CosmosMsg::Wasm(WasmMsg::Execute {
            contract_addr: deps
                .api
                .addr_validate(stable_pair_info.liquidity_token.as_ref())?
...
pub fn query_stable_pool(deps: Deps) -> Result<StablePoolResponse, ContractError> {
    ...
    let assets_address: Vec<String> = pair_info
        .asset_infos
        .iter()
        .map(|asset_info| asset_info.to_normal(deps.api).unwrap().to_string())
        .collect::<Vec<String>>();
...
pub fn query_withdraw_liquidity_by_share_simulation(
    ...
    // Return the assets that user will receive
    Ok(pairs
        .iter()
        .zip(assets_amount.iter())
        .map(|(pair, asset_amount)| Asset {
            info: pair.info.clone(),
            amount: *asset_amount,
        })
        .collect::<Vec<Asset>>())
}
